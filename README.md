...
Hush is a single-file notes app with folders, Markdown, search, and a built-in space to think out loud about each note. No account, no server, no tracking, no build step. Open the page, set a passphrase, write.
...
2. Your notes are stored as a single encrypted blob in your browser's local storage.
3. When you unlock, the blob is decrypted in memory only. When you lock (or after 10 minutes of inactivity), the key is dropped.
4. **Backup and vault** gives you the encrypted blob as text you can copy somewhere safe, and restore on any device.
...
- Keeps decrypted notes and the key in memory only while unlocked.
...
1. On the old device: **Backup and vault**, then **Copy backup**.
2. On the new device: on the lock screen choose **Restore from backup**, paste, then unlock with the same passphrase.
